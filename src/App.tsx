...
import { RichTextToolbar } from './components/EmailBuilder/RichTextToolbar/RichTextToolbar';
...
const AppContent: React.FC = () => {
  ...
  const isTextElementSelected = () => {
    ...
    const elementInfo = findElement(selectedElementId);
    return elementInfo?.element.type === 'text';
  };

  // 生成唯一的工具栏和编辑器 ID
  const toolbarId = isTextElementSelected() ? `rich-text-toolbar-${selectedElementId}` : undefined;
  const editorId = isTextElementSelected() ? `text-editor-${selectedElementId}` : undefined;
  ...
  return (
    ...
    >
      <div className="app">
        ...
        {isTextElementSelected() && toolbarId && editorId && (
          <RichTextToolbar toolbarId={toolbarId} editorId={editorId} />
        )}
